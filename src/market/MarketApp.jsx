...
import {
  ...
  marketUrunSatisGorunurlugunuGuncelle,
...
const bosUrun = {
  ...
  stokMiktari: '', minimumStok: '', rafKonumu: '', sonKullanmaTarihi: '', lotNo: '', resimUrl: '', satisEkranindaGoster: true,
};
...
export default function MarketApp({ restaurantId, restaurantName, currentUserName, notify, canPerform }) {
  ...
  const [genelIndirimTaslagi, setGenelIndirimTaslagi] = useState({ yon: 'azalt', deger: '' });
  ...
  const etiketBarkodRef = useRef(null);
  ...
  const genelIndirimTaslakDegeri = Math.max(Number(genelIndirimTaslagi.deger || 0), 0);
  const genelIndirimTaslakYuzdeTutari = paraYuvarla(
    sepetToplamlari.araToplam * Math.min(genelIndirimTaslakDegeri, 100) / 100
  );
  const genelIndirimTaslakTlTutari = paraYuvarla(genelIndirimTaslagi.yon === 'arttir'
    ? genelIndirimTaslakDegeri
    : Math.min(genelIndirimTaslakDegeri, sepetToplamlari.araToplam));
  const genelIndirimTaslakSonucu = tutar => paraYuvarla(Math.max(
    sepetToplamlari.araToplam + (genelIndirimTaslagi.yon === 'arttir' ? tutar : -tutar),
    0
  ));
  ...
  const etiketBarkodHaritasi = useMemo(
    () => new Map(urunler.flatMap(urun => [urun.barkod, urun.stok_kodu]
      .map(kod => [String(kod || '').trim(), urun])
      .filter(([kod]) => kod))),
    [urunler]
  );
  ...
  const filtreliEtiketUrunleri = useMemo(() => {
    ...
    const gorunumUrunleri = etiketGorunumu === 'degisenler'
      ...
      : urunler;
    const filtreliListe = !metin ? gorunumUrunleri : gorunumUrunleri.filter(urun => [urun.urun_adi, urun.barkod, urun.stok_kodu]
      .some(value => String(value || '').toLocaleLowerCase('tr-TR').includes(metin)));
    if (!etiketSeciliUrunIdSeti.size) return filtreliListe;
    const seciliUrunler = [];
    const digerUrunler = [];
    filtreliListe.forEach(urun => (etiketSeciliUrunIdSeti.has(String(urun.id)) ? seciliUrunler : digerUrunler).push(urun));
    return [...seciliUrunler, ...digerUrunler];
  }, [bekleyenEtiketUrunIdSeti, etiketGorunumu, etiketSeciliUrunIdSeti, gecikmeliEtiketArama, urunler]);
  ...
  const urunuDuzenle = urun => {
    setUrunFormu({
      ...
      satisEkranindaGoster: urun.satis_ekraninda_goster !== false,
    });
  ...
  const urunSatisGorunurlugunuDegistir = async urun => {
    if (!yetkiyiDogrula('urun_yonet', 'Ürünün satış ekranı görünürlüğünü değiştirmek için ürün yönetme yetkisi gerekir.')) return;
    const yeniDurum = urun.satis_ekraninda_goster === false;
    try {
      const guncellenen = await marketUrunSatisGorunurlugunuGuncelle(restaurantId, urun.id, yeniDurum);
      const guncelle = liste => liste.map(item => String(item.id) === String(urun.id)
        ? { ...item, ...guncellenen, satis_ekraninda_goster: yeniDurum }
        : item);
      setUrunler(guncelle);
      setTumUrunler(guncelle);
      bildir(yeniDurum
        ? `${urun.urun_adi} satış ekranında gösterilecek.`
        : `${urun.urun_adi} kutulardan gizlendi; barkodla satılabilir.`, 'success');
    } catch (error) {
      bildir(error.message, 'error');
    }
  };
  ...
  const sonrakiBekleyenFisAdi = () => {
    const kullanilanNumaralar = bekleyenSepetler
      .map(kayit => String(kayit.sepet_adi || '').trim().match(/^Fiş\s+(\d+)$/i))
      .filter(Boolean)
      .map(eslesme => Number(eslesme[1]))
      .filter(Number.isFinite);
    return `Fiş ${Math.max(0, ...kullanilanNumaralar) + 1}`;
  };

  const sepetiBeklemeyeAl = async (event, sepetAdi = '') => {
    event?.preventDefault();
    ...
    try {
      ...
      const kayit = await marketBekleyenSepetiKaydet(restaurantId, {
        sepetAdi: String(sepetAdi || '').trim() || sonrakiBekleyenFisAdi(),
        cariId: satisCariId,
  ...
  const genelIndirimPenceresiniAc = () => {
    const kayitliAyarVar = Number(genelIndirim.deger || 0) > 0;
    setGenelIndirimTaslagi({
      yon: kayitliAyarVar ? genelIndirim.yon : yetkiVar('indirim_yap') ? 'azalt' : 'arttir',
      deger: kayitliAyarVar ? String(genelIndirim.deger) : '',
    });
    setGenelIndirimPenceresi(true);
  };

  const genelIndirimTusunaBas = tus => {
    setGenelIndirimTaslagi(onceki => {
      const mevcut = String(onceki.deger || '').replace(',', '.').replace(/[^\d.]/g, '');
      if (tus === 'C') return { ...onceki, deger: '' };
      if (tus === 'sil') return { ...onceki, deger: mevcut.slice(0, -1) };
      if (tus === '.') return { ...onceki, deger: mevcut.includes('.') ? mevcut : `${mevcut || '0'}.` };

      const eklenecek = String(tus).replace(/\D/g, '');
      if (!eklenecek) return onceki;
      if (mevcut.includes('.')) {
        const ondalikUzunlugu = mevcut.split('.')[1]?.length || 0;
        return { ...onceki, deger: `${mevcut}${eklenecek.slice(0, Math.max(2 - ondalikUzunlugu, 0))}` };
      }
      const yeniDeger = mevcut === '0' ? eklenecek.replace(/^0+/, '') || '0' : `${mevcut}${eklenecek}`;
      return { ...onceki, deger: yeniDeger.slice(0, 8) };
    });
  };

  const genelIndirimiUygula = tur => {
    if (!odemeSurerkenSepetDegisebilirMi()) return;
    const deger = Number(genelIndirimTaslagi.deger);
    if (!Number.isFinite(deger) || deger <= 0) return bildir('Sıfırdan büyük bir indirim veya artış değeri girin.', 'warning');
    if (tur === 'yuzde' && deger > 100) return bildir('Yüzde değeri 100’den büyük olamaz.', 'warning');
    const yetkili = genelIndirimTaslagi.yon === 'arttir'
    ...
    if (!yetkili) return;
    const uygulanacakDeger = tur === 'tutar' && genelIndirimTaslagi.yon !== 'arttir'
      ? Math.min(deger, sepetToplamlari.araToplam)
      : deger;
    setGenelIndirim({ yon: genelIndirimTaslagi.yon, tur, deger: String(uygulanacakDeger) });
    setGenelIndirimPenceresi(false);
    window.setTimeout(() => barkodRef.current?.focus(), 80);
  };
  ...
  const etiketAramasindaEnter = event => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    const barkod = String(event.currentTarget.value || etiketArama).trim();
    if (!barkod) return;
    ...
    setEtiketAdetleri(prev => ({
      ...prev,
      [urun.id]: Number(prev[urun.id] || 0) + 1,
    }));
    setEtiketGorunumu('tumu');
    setEtiketListeSayfasi(1);
    setEtiketArama('');
    window.setTimeout(() => etiketBarkodRef.current?.focus({ preventScroll: true }), 0);
  };
    ...
    <section className="market-shell">
      ...
      {!yukleniyor && sekme === 'satis' && <div className="market-pos-layout market-pos-practical">
        ...
        <div className="market-card market-checkout">
          ...
          <div className="market-park-toolbar">
            {yetkiVar('adisyon_duzenle') && <button type="button" disabled={!sepet.length || bekleyenSepetIsleniyor} onClick={() => void sepetiBeklemeyeAl()}><kbd>F5</kbd> ⏸ Sepeti Beklet</button>}
          ...
          <aside className="market-pos-action-rail" aria-label="Hızlı satış işlemleri">
            ...
            {yetkiVar('adisyon_duzenle') && <button type="button" disabled={!sepet.length || bekleyenSepetIsleniyor || odemeBasladi} onClick={() => void sepetiBeklemeyeAl()}><span>⏸</span><b>Beklet</b><small>F5</small></button>}
        ...
        {genelIndirimPenceresi && <div className="market-price-modal" role="dialog" aria-modal="true" aria-label="Sepet toplam fiyat ayarı">
          <form className="market-total-adjust-modal market-touch-discount-modal" onSubmit={event => event.preventDefault()}>
            <header className="market-touch-discount-head">
              <span><small>TOPLAM FİYAT AYARI</small><strong>İndirim / artış gir</strong></span>
              <span className="market-touch-discount-head-actions">
                {Number(genelIndirim.deger || 0) > 0 && <button type="button" className="reset" onClick={() => { setGenelIndirim({ yon: 'azalt', tur: 'yuzde', deger: '' }); setGenelIndirimPenceresi(false); }}>Ayarı kaldır</button>}
                <button type="button" className="close" aria-label="Fiyat ayarını kapat" title="Kapat" onClick={() => setGenelIndirimPenceresi(false)}>×</button>
              </span>
            </header>
            <div className="market-adjust-direction">
              <button type="button" className={genelIndirimTaslagi.yon !== 'arttir' ? 'active decrease' : ''} onClick={() => setGenelIndirimTaslagi({ ...genelIndirimTaslagi, yon: 'azalt' })}>− Azalt</button>
              <button type="button" className={genelIndirimTaslagi.yon === 'arttir' ? 'active increase' : ''} onClick={() => setGenelIndirimTaslagi({ ...genelIndirimTaslagi, yon: 'arttir' })}>＋ Artır</button>
            </div>
            <label className="market-touch-discount-value">TUŞLANAN DEĞER<input type="number" min="0" step="0.01" inputMode="decimal" value={genelIndirimTaslagi.deger} onChange={event => setGenelIndirimTaslagi({ ...genelIndirimTaslagi, deger: event.target.value })} placeholder="0,00" autoFocus /></label>
            <div className="market-standard-percentages" aria-label="Hazır indirim yüzdeleri">
              {[5, 10, 15, 20].map(yuzde => <button type="button" key={yuzde} className={Number(genelIndirimTaslagi.deger) === yuzde ? 'active' : ''} onClick={() => setGenelIndirimTaslagi({ ...genelIndirimTaslagi, deger: String(yuzde) })}>%{yuzde}</button>)}
            </div>
            <div className="market-scale-keypad market-misc-price-keypad market-discount-keypad" aria-label="Dokunmatik indirim tuş takımı">
              {[7, 8, 9].map(rakam => <button type="button" key={rakam} onClick={() => genelIndirimTusunaBas(rakam)}>{rakam}</button>)}<button type="button" className="control" onClick={() => genelIndirimTusunaBas('sil')}>⌫</button>
              {[4, 5, 6].map(rakam => <button type="button" key={rakam} onClick={() => genelIndirimTusunaBas(rakam)}>{rakam}</button>)}<button type="button" className="control" onClick={() => genelIndirimTusunaBas('C')}>C</button>
              {[1, 2, 3, 0].map(rakam => <button type="button" key={rakam} onClick={() => genelIndirimTusunaBas(rakam)}>{rakam}</button>)}
              <button type="button" className="wide" onClick={() => genelIndirimTusunaBas('00')}>00</button><button type="button" className="wide" onClick={() => genelIndirimTusunaBas('.')}>Virgül</button>
            </div>
            <section className="market-touch-discount-summary">
              <span>Sepet toplamı<strong>{para(sepetToplamlari.araToplam)}</strong></span>
              <span>İşlem<strong>{genelIndirimTaslagi.yon === 'arttir' ? 'Artırılacak' : 'Azaltılacak'}</strong></span>
            </section>
            <div className="market-discount-type-actions">
              <button type="button" className="percent" disabled={!genelIndirimTaslakDegeri} onClick={() => genelIndirimiUygula('yuzde')}>
                <span>% UYGULA</span>
                <strong>{genelIndirimTaslagi.yon === 'arttir' ? '+' : '−'}{para(genelIndirimTaslakYuzdeTutari)}</strong>
                <small>Yeni toplam {para(genelIndirimTaslakSonucu(genelIndirimTaslakYuzdeTutari))}</small>
              </button>
              <button type="button" className="amount" disabled={!genelIndirimTaslakDegeri} onClick={() => genelIndirimiUygula('tutar')}>
                <span>TL UYGULA</span>
                <strong>{genelIndirimTaslagi.yon === 'arttir' ? '+' : '−'}{para(genelIndirimTaslakTlTutari)}</strong>
                <small>Yeni toplam {para(genelIndirimTaslakSonucu(genelIndirimTaslakTlTutari))}</small>
              </button>
            </div>
          </form>
        </div>}
      ...
      {!yukleniyor && sekme === 'urunler' && <div className="market-grid-form">
        <form className="market-card market-form market-product-form" onSubmit={urunKaydet}>
          ...
          <label className="market-check market-product-visibility-check"><input type="checkbox" checked={urunFormu.satisEkranindaGoster !== false} onChange={event => setUrunFormu({ ...urunFormu, satisEkranindaGoster: event.target.checked })} /> Barkodlu satış ekranında ürün kutusu olarak göster <small>Kapalı ürün kutularda görünmez; barkodu okutulunca satışa eklenir.</small></label>
          ...
          <div className="market-table"><table><thead><tr><th>Barkod / Ürün</th><th>Grup</th><th>Stok</th><th>Alış</th><th>Satış</th><th>Kâr</th><th></th></tr></thead><tbody>
            {sayfadakiUrunler.map(urun => {
              ...
              return <Fragment key={urun.id}>
                <tr className={urun.satis_ekraninda_goster === false ? 'market-product-hidden-row' : ''}><td><strong>{urun.urun_adi}</strong><small>{urun.barkod}{urun.son_kullanma_tarihi ? ` · SKT ${tarihYaz(urun.son_kullanma_tarihi)}` : ''}{urun.lot_no ? ` · Lot ${urun.lot_no}` : ''}</small></td><td>{urun.kategori}</td><td className={kritikUrunMu(urun) ? 'red' : ''}>{urun.stok_miktari} {urun.birim}</td><td>{para(urun.alis_fiyati)}</td><td><strong>{para(urun.satis_fiyati)}</strong></td><td className={kar < 0 ? 'red' : 'green'}>{para(kar)}</td><td><div className="market-inline-actions"><button type="button" title="Ürünü satış ekranında sola taşı" aria-label={`${urun.urun_adi} ürününü sola taşı`} disabled={urunSiraBilgisi.index <= 0 || Boolean(siralamaKaydediliyor)} onClick={() => satisSirasiniDegistir('urun', urun, -1)}>←</button><button type="button" title="Ürünü satış ekranında sağa taşı" aria-label={`${urun.urun_adi} ürününü sağa taşı`} disabled={urunSiraBilgisi.index === urunSiraBilgisi.toplam - 1 || Boolean(siralamaKaydediliyor)} onClick={() => satisSirasiniDegistir('urun', urun, 1)}>→</button><button type="button" className={urun.satis_ekraninda_goster === false ? 'market-product-visibility-button hidden' : 'market-product-visibility-button active'} onClick={() => urunSatisGorunurlugunuDegistir(urun)}>{urun.satis_ekraninda_goster === false ? '⊘ Gizli' : '👁 Ekranda'}</button><button type="button" title="Stok ve fiyatı düzenle" aria-label={`${urun.urun_adi} stok ve fiyatını düzenle`} onClick={() => acik ? setHizliDuzenleme(null) : hizliDuzenlemeyiAc(urun)}>✎</button><button type="button" onClick={() => urunuDuzenle(urun)}>Detay</button>{yetkiVar('silme_yap') && <button className="market-product-delete" type="button" disabled={String(silinenUrunId) === String(urun.id)} onClick={() => urunuSil(urun)}>{String(silinenUrunId) === String(urun.id) ? 'Siliniyor…' : 'Sil'}</button>}</div></td></tr>
      ...
      {!yukleniyor && sekme === 'etiket' && <div className="market-card">
        ...
        <input ref={etiketBarkodRef} className="market-label-search" value={etiketArama} onChange={event => { setEtiketArama(event.target.value); setEtiketListeSayfasi(1); }} onKeyDown={etiketAramasindaEnter} placeholder="Barkodu okutun ve Enter'a basın veya ürün adıyla arayın" autoFocus />
